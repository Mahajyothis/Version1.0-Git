...
function chatHeartbeat(){
	...
	$.ajax({
	  url: "<?php echo base_url(); ?>chat?action=chatheartbeat",
	  cache: false,
	  dataType: "json",
...
function closeChatBox(chatboxtitle) {
	...
	$.post("<?php echo base_url(); ?>chat?action=closechat", { chatbox: chatboxtitle} , function(data){	
	});
...
function checkChatBoxInputKey(event,chatboxtextarea,chatboxtitle,chatname) {
	...
	if(event.keyCode == 13 && event.shiftKey == 0)  {
		...
		$("#chatbox_"+chatboxtitle+" .chatboxcontent").append('<div class="chatboxmessage"><span class="chatboxmessagefrom">'+'me'+':&nbsp;&nbsp;</span><span class="chatboxmessagecontent">'+message+'</span></div>');
			$.post("<?php echo base_url(); ?>chat?action=sendchat", {to: chatboxtitle, message: message} , function(data){
...
function startChatSession(){ 
	...
	$.ajax({
	  url: "<?php echo base_url(); ?>chat?action=startchatsession",
	  cache: false,
	  dataType: "json",
	  async: false,
	  success: function(data) {
...
//alert(username);
	...
	$.ajax({
	  url: "<?php echo base_url(); ?>chat?action=chatname&usw="+item.f,
	  cache: false,
	  dataType: "json",
	  async: false,
