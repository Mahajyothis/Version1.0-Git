...
function _setjsmlfolder() {
	if (window.jsml)
		jsml.jsmlfolder = MyLiveChat.SiteUrl + "JSML";
	else
		setTimeout(_setjsmlfolder, 100);
}
_setjsmlfolder();

function MyLiveChat_InitOnce() {

	for (var mlcp in MyLiveChat.RawLangJson) {
		MyLiveChat["TEXT_" + mlcp.toUpperCase()] = MyLiveChat.RawLangJson[mlcp];
	}
	...
	MyLiveChat.InviteHideTimeout = MyLiveChat._GetNumber(MyLiveChat.InviteHideTimeout, 1800);
	...
	MyLiveChat.SmartUseLocationFilter = ("0" != MyLiveChat.SmartUseLocationFilter);
	...
	MyLiveChat.InlineChatIntroSize = MyLiveChat.InlineChatIntroSize || "13px";
...
function MyLiveChat_CalcImagePath(name, defval, ext) {
	...
	var val = (MyLiveChat[name] || defval || "1");
	if (val.indexOf("//") != -1)
		return;
	var pngFiles = ",invitephoto_1,invitephoto_7,invitephoto_8,buttononlineimage_2,buttononlineimage_3,buttononlineimage_4," +
		"buttononlineimage_5,buttononlineimage_6,buttononlineimage_7,buttononlineimage_8,buttononlineimage_17,\
buttononlineimage_18,buttononlineimage_19,buttononlineimage_20," +
		"buttonofflineimage_2,buttonofflineimage_3,buttonofflineimage_4," +
		"buttonofflineimage_5,buttonofflineimage_6,buttonofflineimage_7,buttonofflineimage_8,buttonofflineimage_17,\
buttonofflineimage_18,buttonofflineimage_19,buttonofflineimage_20," +
		"smartphoto_1,smartphoto_2,smartphoto_3,smartphoto_9,smartphoto_10,";
	var file = name + "_" + val;
...
function MyLiveChat_CalcCustomUrl(name, part2) {
	...
	if (MyLiveChat[name + part2] != "custom") {
		...
		if (name == "InPageBubbleTop") {
			var ix = (MyLiveChat[name + part2] || "1");
			if (ix == "3" || ix == "4" || ix == "5") {
				ix = MyLiveChat.HasReadyAgents ? "Online_" + ix : "Offline_" + ix;
				MyLiveChat[name + "Url" + part2] = MyLiveChat.SiteUrl + "Customization/Template/" + name + part2 + "_" + ix + ".png";
			}
		}
	}
}
...
function MyLiveChat_InitOnceDialog() {
	...
	MyLiveChat.SupportShowFlashTitle = MyLiveChat.SupportShowFlashTitle != "0";
...
function MyLiveChat_InitOnceInline() {
	...
	MyLiveChat.OnholdWaitTime = MyLiveChat.InlineChatOnholdWaitTime;
}

function MyLiveChat_InitOnceFinal() {

	if (MyLiveChat.RawConfig.CustomDataDefinition) {
		MyLiveChat_SetCustomData(MyLiveChat.RawConfig.CustomDataDefinition);
	}
	else {
		MyLiveChat_SetCustomData(MyLiveChat.SourceCustomData);
	}

	if (MyLiveChat.RawQuery.Department) {
		MyLiveChat.SourceDepartment = MyLiveChat.RawQuery.Department;
	}
	if (MyLiveChat.RawQuery.CustomData) {
		MyLiveChat_SetCustomData(MyLiveChat.RawQuery.CustomData);
	}


	if (MyLiveChat.HasReadyAgents && MyLiveChat.SourceDepartment && MyLiveChat.LS_ShowAllCustomers == "False" && MyLiveChat.WaitingDepartmentMode != "Auto") {
		for (var di = 0; di < MyLiveChat.Departments.length; di++) {
			var dep = MyLiveChat.Departments[di];
			if (dep.Name == MyLiveChat.SourceDepartment) {
				if (!dep.Online) {
...
function MyLiveChat_IsDuringOperatingHours() {
	...
	var clientTime = new Date(new Date().getTime() + (MyLiveChat.OperatingHoursTimeZone - clientTimezone) * 60 * 60 * 1000);
	var clientHour = clientTime.getHours();
	var clientMinute = clientTime.getMinutes();

	var scheduleRange = [];
	var dayOfWeek = clientTime.getDay();
	if (MyLiveChat.OperatingHoursSchedule == "0") {
		if ((dayOfWeek > 0 || dayOfWeek < 6) && MyLiveChat.OperatingHoursCheckWeekday && MyLiveChat.OperatingHoursValWeekday)
			scheduleRange = MyLiveChat.OperatingHoursValWeekday.split("-");
		if ((dayOfWeek == 0 || dayOfWeek == 6) && MyLiveChat.OperatingHoursCheckWeekend && MyLiveChat.OperatingHoursValWeekend)
			scheduleRange = MyLiveChat.OperatingHoursValWeekend.split("-");
	}
	else {
		if (MyLiveChat["OperatingHoursCheckDay" + dayOfWeek] && MyLiveChat["OperatingHoursValDay" + dayOfWeek])
			scheduleRange = MyLiveChat["OperatingHoursValDay" + dayOfWeek].split("-");
	}

	if (scheduleRange.length == 0)
		return;

	var ct = clientHour * 60 + clientMinute;

	var st = parseInt(scheduleRange[0].split(":")[0]) * 60 + parseInt(scheduleRange[0].split(":")[1]);
	var et = parseInt(scheduleRange[1].split(":")[0]) * 60 + parseInt(scheduleRange[1].split(":")[1]);
...
function MyLiveChat_GetCookie(name) {
...
function MyLiveChat_SetCookie(name, value, seconds) {
...
function MyLiveChat_GetLocalData(name) {
	...
	return MyLiveChat_GetCookie(name);
}
function MyLiveChat_SetLocalData(name, value, seconds) {
	...
	MyLiveChat_SetCookie(name, value, seconds);
}
...
function MyLiveChat_SmartTriggers() {
	//for this page
	...
	if (MyLiveChat.SmartHideIfChatting) {
		if (MyLiveChat.VisitorStatus && MyLiveChat.VisitorStatus != "VISIT") {
			return false;
		}
	}
	...
	if (MyLiveChat.SmartUseStayTime && MyLiveChat.VisitorDuration + (new Date().getTime() - (MyLiveChat.LastLoadTime || MyLiveChat.PageBeginTime)) / 1000 > MyLiveChat.SmartStayTime) {
		return true;
	}
	...
	if (MyLiveChat.SmartUseLocationFilter && MyLiveChat.SmartLocationFilter) {
		if (MyLiveChat_SmartIsMatch(MyLiveChat.VisitorLocation, MyLiveChat.SmartLocationFilter))
			return true;
	}
...
function MyLiveChat_SmartIsMatch(data, filter) {
	...
	for (var i = 0; i < arr.length; i++) {
		...
		if (MyLiveChat_PatternMatch(item, data))
			return true;
	}
}


function MyLiveChat_PatternMatch(pattern, value) {
	if (!pattern)
		return true;//skip empty pattern
	if (!value)
		return false;

	if (pattern.substring(0, 7) == "regexp:") {
		try {
			var re = new RegExp(pattern.substring(7), "ig");
	...
	for (var i = 0; i < parts.length; i++) {
		...
		if (!part.length)
			continue;
		index = value.indexOf(part, index);
		if (index == -1)
			return false;
		index += part.length;
	}
...
function MyLiveChat_PlaySound(url, volume) {
	if (url.indexOf('/') == -1) {
		url = MyLiveChat.SiteUrl + "livechat/Sound/" + url + ".mp3";
	}
	if (window.Audio) {
...
function MyLiveChat_AcceptInvite() {
	...
	if (MyLiveChat_IsUseInlineChat("Invite")) {
	}
	else {
		try {
...
function MyLiveChat_ChangeUrl(_synctype) {
	...
	if (_synctype == "VISIT" && MyLiveChat._RequestingServerInvitation) {
		if (MyLiveChat._RequestingServerInvitationDepartment)
			url += "&RSI=" + encodeURIComponent(MyLiveChat._RequestingServerInvitationDepartment);
		else
			url += "&RSI=1";
	}
...
function MyLiveChat_IsUseInlineChat(mode) {

	if (MyLiveChat.IsMobile)
		return false;

	if (!mode) mode = "Button";

	if (MyLiveChat.InlineChatIsActive()) {

		var setting = MyLiveChat[mode + "OpenNewWindow"];
		if (!setting) {
			if (mode == "Smart")
				setting = "False";
			if (mode == "Invite")
...
function MyLiveChat_OpenDialog(mode) {
	if (MyLiveChat_IsUseInlineChat(mode))
...
function MyLiveChat_Initialize(fromcpr) {
	...
	if (MyLiveChat.OperatingHoursEnable) {
		var isophour = MyLiveChat_IsDuringOperatingHours();
		if (isophour == true) {
			MyLiveChat.HasReadyAgents = true;
		}
		else if (isophour === false && MyLiveChat.OperatingHoursForceOffline) {
			MyLiveChat.HasReadyAgents = false;
		}
	}
	...
	if (MyLiveChat.SyncType == "VISIT") {
		...
		if (!MyLiveChat.HasInvited) {
			if (MyLiveChat.ShowSmart && MyLiveChat_SmartTriggers()) {
				MyLiveChat_InvokeEvent("ShowSmart");
				MyLiveChat_ShowSmart();
			}
			else if (MyLiveChat.SmartPanel) {
				MyLiveChat_HideSmart();
			}
		}
	}
	if (MyLiveChat.SyncType == "ACCEPT") {
		if (MyLiveChat.SyncStatus == "READY") {
			...
			if (MyLiveChat_IsUseInlineChat("Invite")) {
				MyLiveChat.InlineChatStartChat();
			}
			else {
...
function MergeCustomData(oldcustdata, custdata) {
	...
	try {
		if (oldcustdata instanceof Array)
			...
			olddata = eval("_mlctemp_=" + oldcustdata);
	}
	catch (x) {
	}
	if (!olddata && oldcustdata) {
		olddata = [{ text: "Custom", value: oldcustdata }]
	}
	var newdata = null;
	try {
		if (custdata instanceof Array)
			newdata = custdata.concat();
		else
			newdata = eval("_mlctemp_=" + custdata);
	}
	catch (x) {
	}
	if (!newdata && custdata) {
...
function MyLiveChat_InlineTriggerMatch(item) {
	...
	if (item.department) {
		for (var di = 0; di < MyLiveChat.Departments.length; di++) {
			var dep = MyLiveChat.Departments[di];
			if (dep.Name.toLowerCase() != item.department.toLowerCase())
				continue;
			if (!dep.Online)
				return false;
			break;
	...
	var wt = parseInt(item.waittime);
	if (wt && wt > 0 && new Date().getTime() - MyLiveChat.PageBeginTime < wt * 1000)
		return false;

	if (!MyLiveChat_PatternMatch(item.urlpattern, location.href))
		return false;
	if (!MyLiveChat_PatternMatch(item.refpattern, MyLiveChat.VisitorReferUrl))
		return false;
	if (!MyLiveChat_PatternMatch(item.locationkey, MyLiveChat.VisitorLocation))
		return false;

	MyLiveChat.InlineTriggerDepartment = item.department;
	MyLiveChat.InlineTriggerMessage = item.message;
	MyLiveChat_SetLocalData("InlineTriggerMessage", item.message)
	MyLiveChat_SetLocalData("InlineTriggerDepartment", item.department);

	return true;
}

function MyLiveChat_MatchServerTriggers() {
	var _mlctemp_;
	var arr;
	try {
		arr = eval("_mlctemp_=" + MyLiveChat.InlineTriggerDefinition);
	}
	catch (x) {
	}

	if (!arr || !arr.length)//no rules
		return;

	for (var i = 0; i < arr.length; i++) {
		if (MyLiveChat_InlineTriggerMatch(arr[i])) {
			return true;
		}
	}
}
...
function MyLiveChat_TryServerInvitation() {
	...
	if (MyLiveChat._RejectInPage)
		return;
	if (!MyLiveChat.HasReadyAgents)
		return;
	if (MyLiveChat.InlineChatIsExpanded())
		return;

	if (MyLiveChat.VisitorStatus == "SERVERINVITE") {

	}
	else if (MyLiveChat.VisitorStatus != "VISIT") {
		return;
	}
	else {
		//away
		var uadt = parseInt(MyLiveChat_GetLocalData("uadt")) || 0;
		if (uadt && new Date().getTime() - uadt > 5 * 60 * 1000)
			return;

		var timeout = MyLiveChat.InviteHideTimeout;
		if (timeout > 0 && new Date().getTime() - Math.max(10, timeout) * 1000 < parseInt(MyLiveChat_GetLocalData("cpmsitime")) || 0)
			return;


		if (!MyLiveChat_MatchServerTriggers())
			return;
	}


	MyLiveChat_SetLocalData("cpmsitime", new Date().getTime());
	MyLiveChat._RequestingServerInvitation = 1;
	MyLiveChat._RequestingServerInvitationDepartment = MyLiveChat.InlineTriggerDepartment || MyLiveChat_GetLocalData("InlineTriggerDepartment");
	MyLiveChat_SetNextReqTimeout(1);

}

function MyLiveChat_OnServerInvitation(agentid, agentname) {

	MyLiveChat_SetLocalData("cpmsitime", new Date().getTime());
	MyLiveChat_SetLocalData("cpmsiaid", agentid);
	MyLiveChat_SetLocalData("cpmsiadn", agentname);

	MyLiveChat_RestoreServerInvite(true);
}
function MyLiveChat_RestoreServerInvite(bysync) {
	...
	if (!bysync && MyLiveChat.VisitorStatus != "SERVERINVITE") {
		if (!MyLiveChat.VisitorStatus) {
			MyLiveChat._RequestingServerInvitation = 1;
			MyLiveChat._RSID = MyLiveChat.InlineTriggerDepartment || MyLiveChat_GetLocalData("InlineTriggerDepartment");
			MyLiveChat_SetNextReqTimeout(1);
		}
		return;
	}

	MyLiveChat.IsByServerInvitation = true;
	MyLiveChat.InlineTriggerAgentId = MyLiveChat_GetLocalData("cpmsiaid");
	MyLiveChat.InlineTriggerAgentName = MyLiveChat_GetLocalData("cpmsiadn");
	MyLiveChat.InlineTriggerMessage = MyLiveChat.InlineTriggerMessage || MyLiveChat_GetLocalData("InlineTriggerMessage");
	if (MyLiveChat.InlineChatIsActive()) {
		MyLiveChat.InlineChatStartChat();
		if (bysync) MyLiveChat.SetInlineChatAction("trigger");
	}
	else {
	...
	function autoreject() {
		if (!MyLiveChat.InlineChatIsExpanded() || MyLiveChat.InlineChatIsChatting() || MyLiveChat._InlineAction != "trigger")
			return;

		var rejectit = false;


		if (MyLiveChat.InviteShowTimeout > 0 && new Date().getTime() - MyLiveChat.InviteShowTimeout * 1000 > parseInt(MyLiveChat_GetLocalData("cpmsitime")) || 0)
			rejectit = true;
		...
		if (rejectit) {
			MyLiveChat._RejectInPage = true;
			MyLiveChat.InlineChatCloseChat(true);
		}
		else {
			setTimeout(autoreject, 2000);
		}
	}
...
MyLiveChat.InlineChatIsActive = function () {
}
MyLiveChat.InlineChatIsExpanded = function () {
}
MyLiveChat.InlineChatIsCollapsed = function () {
}
MyLiveChat.InlineChatDoExpand = function () {
}
MyLiveChat.InlineChatDoCollapse = function () {
}
MyLiveChat.InlineChatIsChatting = function () {
}
MyLiveChat.InlineChatStartChat = function () {
}
MyLiveChat.InlineChatCloseChat = function (noask) {
}

MyLiveChat.SetInlineChatAction = function (name, args) {
}
...
function CreateCrossPageManager(control) {
	...
	var localdataok = MyLiveChat_SupportLocalData();
	var cpm = MyLiveChat.CrossPageManager = {};

	MyLiveChat.InlineChatUID = String((new Date().getTime() + Math.random()) / 1000 - 1400000000).substr(0, 16);
	trace("UID " + MyLiveChat.InlineChatUID);
	...
	cpm.ExpandInPagePanel = function (startchat) {
		...
		MyLiveChat.SetInlineChatAction("expand");
	}

	function DoCollapseDialog(disposechat, fromsync) {
		if (!fromsync) {
			if (disposechat)
				MyLiveChat.SetInlineChatAction("closechat");
			else
				MyLiveChat.SetInlineChatAction("collapse");
		}
		...
		if (!disposechat)
			return;
		MyLiveChat.FrameWin = null;
		cpm.Expanded = false;
		control.invoke_recursive("disposechat");
		if (window.dispose_cobrowsedialog) window.dispose_cobrowsedialog();
	}
	cpm.CollapseInPagePanel = function (disposechat, force) {
		if (!dialogexpanded)
			return;
		if (MyLiveChat.FrameWin.MyLiveChat.SurveyVisible && disposechat && !force) {
			MyLiveChat.FrameWin.surveypanel.call_show(function () {
				DoCollapseDialog(disposechat);
			});
		}
		else {
			DoCollapseDialog(disposechat);
	...
	MyLiveChat.InlineChatDoExpand = function () {
		if (dialogexpanded) return;
		cpm.ExpandInPagePanel();
	}
	MyLiveChat.InlineChatDoCollapse = function () {
		if (!dialogexpanded) return;
		cpm.CollapseInPagePanel();
	}
	MyLiveChat.InlineChatIsChatting = function () {
		return MyLiveChat.FrameWin && MyLiveChat.FrameWin.IsConnected();
	}
	MyLiveChat.InlineChatStartChat = function () {
		cpm.ExpandInPagePanel(true);
	}
	MyLiveChat.InlineChatCloseChat = function (force) {
		cpm.CollapseInPagePanel(true, force);
	}
	...
	control.mindialog = function (doclosechat) {

		if (doclosechat && MyLiveChat.FrameWin && MyLiveChat.FrameWin.IsConnected()) {
			if (MyLiveChat.FrameWin.MyLiveChat.SurveyVisible) {
				cpm.CollapseInPagePanel(true, false);
			}
			else {
				mlcconfirm({
					message: (MyLiveChat.TEXT_CS_INLINECONFIRM_QUESTION || "Do you want to exit the chat session?"), callback: function (val) {
						if (val) {
							cpm.CollapseInPagePanel(true, true);
						}
		...
		else if (MyLiveChat._InlineAction == "trigger") {
			MyLiveChat.SetInlineChatAction("reject");
			MyLiveChat._RejectInPage = true;
			MyLiveChat._RequestingServerInvitation = null;
			MyLiveChat.IsByServerInvitation = false;
			MyLiveChat_SetNextReqTimeout(1);
			control._mindialog();
			dialogexpanded = false;
		}
		else if (dialogexpanded) {
			MyLiveChat.SetInlineChatAction("collapse");
			control._mindialog();
			dialogexpanded = false;
		}
	...
	MyLiveChat.SetInlineChatAction = function (name, args) {
		if (name == "expand" || name == "collapse") {
			switch (MyLiveChat._InlineAction) {
				case "connected":
				case "chatting":
				case "expand":
				case "collapse":
		...
		if (name == "connected" && MyLiveChat._InlineAction == "collapse")
			name = "collapse";
		var ps = MyLiveChat.InlineChatUID + "|" + new Date().getTime() + "|" + name + "|" + args;
		trace(ps);
		MyLiveChat_SetLocalData("actionparams", ps);
		MyLiveChat._InlineAction = name;
		CheckNextInlineAction();
	}
	...
	var caid;
	function CheckNextInlineAction() {

		clearTimeout(caid);
		caid = setTimeout(CheckNextInlineAction, 1000);

		MyLiveChat._InlineAction = null;
		var ps = MyLiveChat_GetLocalData("actionparams");
		if (!ps) {
			MyLiveChat_TryServerInvitation();
			return;
		}
		ps = ps.split('|');
		var actiondtv = parseInt(ps[1]);
		var name = ps[2];
		var span = new Date().getTime() - actiondtv;
		if (span > 35000 || (name == "reject" && span > 12000)) {
			MyLiveChat_TryServerInvitation();
			return;
		}
		MyLiveChat._InlineAction = name;
		if (padtv >= actiondtv) {
			if (span > 3000) {
				var sname = name == "connected" ? "chatting" : name;
				switch (sname) {
					case "chatting":
					case "expand":
					case "collapse":
						MyLiveChat.SetInlineChatAction(sname);
						break;
					case "trigger":
						MyLiveChat_RestoreServerInvite();
						if (MyLiveChat.IsByServerInvitation)
							MyLiveChat.SetInlineChatAction("trigger");
						return;
				}
			}
			return;
		...
		padtv = actiondtv;
		if (ps[0] != MyLiveChat.InlineChatUID)
			trace(ps);
		var args = ps[3];
		switch (name) {
			case "startwait":
			...
			case "collapse":
				if (name == "collapse")
					MyLiveChat.InlineChatDoCollapse();
				else
					MyLiveChat.InlineChatDoExpand();
				if (!MyLiveChat.InlineChatIsChatting()) {
					control.invoke_recursive("startchat");
					MyLiveChat._ShallResume = true;
				}
				break;
			case "closechat":
				DoCollapseDialog(true, true);
				break;
			case "trigger":
				MyLiveChat_RestoreServerInvite();
				return;
...
MyLiveChat.OnFrameLoad = function (framewin, frameui) {

	MyLiveChat.FrameWin = framewin;
	if (MyLiveChat._ShallResume)
		framewin.promptwait = false;
...
function MyLiveChat_SetCustomData(data) {
	...
	try {
		MyLiveChat.CustomDataDefinition = eval("_mlctemp_=" + (MyLiveChat.SourceCustomData || "null")) || [];
	}
	catch (x) {
...
if (!MyLiveChat.IsDesignMode) {
	...
	if (!MyLiveChat.CCCustomerId || MyLiveChat.CCCustomerId == "null") {
